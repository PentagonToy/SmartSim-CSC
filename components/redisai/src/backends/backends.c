...
int RAI_LoadBackend_JAX(RedisModuleCtx *ctx, const char *path) {
    if (RAI_backends.jax.model_run != NULL) {
        RedisModule_Log(ctx, "warning",
                        "Could not load JAX backend: backend already loaded");
        return REDISMODULE_ERR;
    }
    ...
    if (handle == NULL) {
        RedisModule_Log(ctx, "warning",
                        "Could not load JAX backend from %s: %s",
                        path, dlerror());
        return REDISMODULE_ERR;
    }
    ...
    int (*init_backend)(int (*)(const char *, void **));
    init_backend =
        (int (*)(int (*)(const char *, void **)))(unsigned long)
            dlsym(handle, "RAI_InitBackendJAX");

    if (!_ValidateFuncExists(ctx, init_backend,
                             "RAI_InitBackendJAX", "JAX", path)) {
        goto error;
    }

    if (init_backend(RAI_ExportFunc) != REDISMODULE_OK) {
        RedisModule_Log(ctx, "warning",
                        "Could not initialize JAX backend from %s", path);
        goto error;
    }

    backend.model_create =
        (RAI_Model *(*)(RAI_Backend, const char *, RAI_ModelOpts,
                        const char *, size_t, RAI_Error *))(unsigned long)
            dlsym(handle, "RAI_ModelCreateJAX");

    if (!_ValidateFuncExists(ctx, backend.model_create,
                             "RAI_ModelCreateJAX", "JAX", path)) {
        goto error;
    }

    backend.model_free =
        (void (*)(RAI_Model *, RAI_Error *))(unsigned long)
            dlsym(handle, "RAI_ModelFreeJAX");

    if (!_ValidateFuncExists(ctx, backend.model_free,
                             "RAI_ModelFreeJAX", "JAX", path)) {
        goto error;
    }

    backend.model_run =
        (int (*)(RAI_Model *, RAI_ExecutionCtx **, RAI_Error *))(unsigned long)
            dlsym(handle, "RAI_ModelRunJAX");

    if (!_ValidateFuncExists(ctx, backend.model_run,
                             "RAI_ModelRunJAX", "JAX", path)) {
        goto error;
    }

    backend.model_serialize =
        (int (*)(RAI_Model *, char **, size_t *, RAI_Error *))(unsigned long)
            dlsym(handle, "RAI_ModelSerializeJAX");

    if (!_ValidateFuncExists(ctx, backend.model_serialize,
                             "RAI_ModelSerializeJAX", "JAX", path)) {
        goto error;
    }

    backend.get_version =
        (const char *(*)(void))(unsigned long)
            dlsym(handle, "RAI_GetBackendVersionJAX");

    if (!_ValidateFuncExists(ctx, backend.get_version,
                             "RAI_GetBackendVersionJAX", "JAX", path)) {
        goto error;
    }

    RAI_backends.jax = backend;
    RedisModule_Log(ctx, "notice", "JAX backend loaded from %s", path);
    return REDISMODULE_OK;
...
int RAI_LoadBackend(RedisModuleCtx *ctx, int backend, const char *path) {
    ...
    case RAI_BACKEND_ONNXRUNTIME:
        ...
        break;
    case RAI_BACKEND_JAX:
        ret = RAI_LoadBackend_JAX(ctx, RedisModule_StringPtrLen(fullpath, NULL));
        break;
    }
...
int RAI_LoadDefaultBackend(RedisModuleCtx *ctx, int backend) {
    ...
    case RAI_BACKEND_JAX:
        return RAI_LoadBackend(ctx, backend, "redisai_jax/redisai_jax.so");
    }
