...
RAI_Model *RAI_ModelCreate(RAI_Backend backend, const char *devicestr, RedisModuleString *tag,
    ...
    } else if (backend == RAI_BACKEND_JAX) {
        if (!RAI_backends.jax.model_create) {
            RAI_SetError(err, RAI_EBACKENDNOTLOADED, "ERR Backend not loaded: JAX");
            return NULL;
        }
        model = RAI_backends.jax.model_create(backend, devicestr, opts, modeldef, modellen, err);
    } else {
...
void RAI_ModelFree(RAI_Model *model, RAI_Error *err) {
    ...
    } else if (model->backend == RAI_BACKEND_JAX) {
        if (!RAI_backends.jax.model_free) {
            RAI_SetError(err, RAI_EBACKENDNOTLOADED, "ERR Backend not loaded: JAX");
            return;
        }
        RAI_backends.jax.model_free(model, err);
    } else {
...
int RAI_ModelSerialize(RAI_Model *model, char **buffer, size_t *len, RAI_Error *err) {
    ...
    case RAI_BACKEND_JAX:
        if (!RAI_backends.jax.model_serialize) {
            RAI_SetError(err, RAI_EBACKENDNOTLOADED, "ERR Backend not loaded: JAX");
            return REDISMODULE_ERR;
        }
        ret = RAI_backends.jax.model_serialize(model, buffer, len, err);
        break;
    default:
        RAI_SetError(err, RAI_EUNSUPPORTEDBACKEND, "ERR Unsupported backend");
        return REDISMODULE_ERR;
